...
def read_xyz(file):
    ...
    algo = './ase_ani/DDT/xyzs/aniopt/DDT.xyz'
    fd = open(file, 'r').read()
    rb = re.compile('(\d+?)\n(.*?)\n((?:[A-Z][a-z]?.+?(?:\n|$))+)')
    ra = re.compile(
...
molecules = ['DDT']
ddt_path = './ase_ani/DDT/xyzs/aniopt/DDT.xyz'

for mol in molecules:
    fa = mol + '.xyz'
    X, S, Na, ct = read_xyz('./ase_ani/DDT/xyzs/aniopt/' + fa)
    df = pd.read_csv('./ase_ani/DDT/' + mol + '.csv')
    ani_energies = []
    ...
    pto.plot_2d(phi, psi, Z, xax='PHI', yax='PSI', save='./ase_ani/DDT/'+mol)
